...
We benchmarked ChimeraLM against SACRA~\cite{kiguchi2021longread}, 3rd-ChimeraMiner~\cite{lu2023exploration}, and CADECT~\cite{agyabeng2025evaluating}, existing tools for detecting amplification-induced chimeras (Fig.~\ref{fig:figure2}c,d).
ChimeraLM achieved approximately 90\% reduction in chimeric reads on both platforms, substantially outperforming CADECT (8.3\% and 4.6\% reduction on PromethION and MinION, respectively), while 3rd-ChimeraMiner showed no detectable reduction.
...
It reduced chimeric reads by \(\sim\)90\% while retaining 72--92\% of bulk-supported \glspl{sv}, and it increased supported \gls{sv} calls by 8.5--11.0 fold.
...
In contrast, existing methods showed limited effectiveness on our \gls{ont} \gls{wga} datasets.
SACRA and 3rd-ChimeraMiner, designed for PacBio platforms~\cite{kiguchi2021longread,lu2023exploration}, either failed to complete (SACRA, >500 GB RAM) or showed no detectable reduction (3rd-ChimeraMiner), highlighting poor cross-platform generalization.
...
CADECT uses sliding-window self-alignment to detect concatemers, which are reads containing tandem duplications with internal sequence similarity.
