...
\abstract{Single-cell genomic analysis relies on \gls{wga} to generate sufficient DNA for sequencing, yet this process introduces chimera artifacts that manifest as false-positive \gls{sv} and compromise downstream analyses.
	...
	Evaluated on matched \gls{wga} and bulk nanopore datasets, ChimeraLM eliminated ${\sim}$90\% of chimeric reads from \gls{wga} data, restoring chimeric read proportion to near-bulk levels, whereas existing approaches achieved at most an 8\% reduction.
	...
	Attention-based interpretation indicates that ChimeraLM concentrates classification evidence at chimeric junctions, demonstrating its ability to learn biologically interpretable features.
	ChimeraLM provides a general approach for suppressing amplification-induced artifacts, enabling more reliable single-cell \gls{sv} analysis across long-read platforms.}
...
A major source of error in \gls{wga} is amplification-induced chimera formation.
During this process, highly processive polymerases such as phi29, which is used in \gls{mda}, can switch templates and join discontinuous genomic loci into a single molecule~\cite{dean2002comprehensive,lu2023chimera,lu2023exploration,lasken2007mechanism,agyabeng2025evaluating}.
As a result, \gls{wga}-based sequencing often produces chimeric reads that constitute a substantial fraction of the data~\cite{lu2023chimera}.
These artificial sequences frequently create alignment patterns that closely resemble those generated by genuine \glspl{sv}, including translocations and inversions~\cite{lasken2007mechanism}.
Consequently, \gls{sv} callers that rely on alignment-based signals (e.g., split-read and supplementary alignments) and coverage-derived evidence often misinterpret these amplification artifacts as true rearrangements, inflating false positives and distorting \gls{sv} spectra~\cite{Sedlazeck2018,Smolka2024,chen2023deciphering,heller2019svim,jiang2020longreadbased,gonzalez-pena2021accurate,kosugi2019comprehensive,alkan2011genome, hardLongreadWholegenomeAnalysis2023a}.
This problem is particularly consequential for \gls{wga}-based long-read sequencing, which is otherwise well suited for resolving complex \glspl{sv} at sing-cell resolution~\cite{gupta2024Advances,wen2025Singlecell}.
...
ChimeraLM operates as a post‑alignment filtering module within the single‑cell long‑read sequencing pipeline (Fig.~\ref{fig:figure1}a).
After base calling and alignment to the reference genome, the resulting read set typically contains a mixture of true chimeric reads that arise from authentic genomic rearrangements and artificial chimeras introduced during \gls{wga}.
ChimeraLM evaluates all reads with chimeric alignments before variant calling.
For each read, the model determines whether the observed chimera reflects a genuine genomic event or a \gls{wga}‑induced artifact.
This binary classification enables selective removal of artificial chimeric reads while preserving true rearrangements, which improves the accuracy and sensitivity of downstream \gls{sv} analyses.

To construct a robust supervised training set, we generated \gls{wga} long-read sequencing data from the human prostate cancer cell line PC3 using the \gls{ont} PromethION platform.
For ground-truth calibration, we acquired three matched bulk long-read datasets from unamplified genomic DNA across diverse technologies: \gls{ont} PromethION, \gls{ont} MinION, and \gls{pb} Sequel II.
...
Under this classification scheme, \gls{wga} reads with alignment architectures corroborated by bulk evidence were labeled as genuine genomic events, while those lacking bulk support were classified as \gls{wga}-induced artifacts.
To evaluate the model's ability to generalize across sequencing hardware, we generated an independent \gls{wga} dataset on the MinION platform, which was reserved exclusively for testing (Extended Data Fig.~\ref{fig:data_construction}a).
...
To model these labeled reads, ChimeraLM needs to process long, variable-length DNA sequences at single-nucleotide resolution (Fig.~\ref{fig:figure1}c).
...
\begin{figure}[H]
	...
	\caption{{\bf ChimeraLM improves structural variant detection accuracy.}
		...
		\textbf{(f)}~Distribution of \gls{sv} types for PromethION and MinION datasets across bulk samples, unfiltered \gls{wga}, and \gls{wga} with ChimeraLM filtering. Unfiltered \gls{wga} exhibits an excess of \gls{inv} events, which is substantially reduced following ChimeraLM filtering. Other \gls{sv} classes, including \gls{tra}, \gls{del}, \gls{dup}, and \gls{ins}, are also shown.
	}\label{fig:figure3}
\end{figure}

\subsection*{ChimeraLM reduces unsupported structural variant calls}
Accurate \gls{sv} detection from single cells is essential for characterizing genomic diversity and disease mechanisms. However, \gls{wga}‑induced chimeric artifacts can be misinterpreted as genuine \gls{sv}s, potentially leading to incorrect biological inferences.
To assess the impact of ChimeraLM on \gls{sv} calling, we compared \gls{sv} callsets generated from unfiltered \gls{wga} reads with those obtained after applying ChimeraLM filtering (\gls{wga} + ChimeraLM).
Both callsets were evaluated against two complementary bulk‑derived references: (i) a stringent gold‑standard \gls{sv} set derived from bulk PC3 DNA through cross‑dataset consensus, and (ii) platform‑matched long‑read bulk \gls{sv} callsets used as platform‑specific references.
...
Amplification artifacts can profoundly distort the apparent spectrum of \glspl{sv}, complicating biological interpretation.
To quantify these effects, we compared \gls{sv}‑type compositions across bulk, unfiltered \gls{wga}, and ChimeraLM‑filtered datasets generated on both \gls{ont} PromethION and MinION platforms (Fig.~\ref{fig:figure3}f).
Bulk datasets displayed \gls{sv} profiles dominated by deletions and insertions, whereas \gls{wga} datasets exhibited a striking inflation of inversions, reaching 83.0\% on PromethION and 88.3\% on MinION.
This inversion‑heavy signature mirrors previous observations from single‑cell long‑read sequencing using PacBio and droplet‑based \gls{mda}~\cite{hardLongreadWholegenomeAnalysis2023a}, suggesting that many of these events arise from amplification‑induced chimeras rather than genuine genomic rearrangements.

To directly assess which \gls{sv} types are preferentially associated with amplification artifacts, we examined \gls{sv} calls supported exclusively by reads classified as \gls{wga}‑derived chimeras.
These artifact‑supported events were overwhelmingly enriched for inversions, comprising 88.4\% on PromethION and 92.4\% on MinION (Extended Data Fig.~\ref{fig:sv_artifact_distribution}).
Smaller fractions of deletions, duplications, and insertions were also present, indicating that \gls{wga} artifacts inflate not only inversion calls but generate false positives across multiple \gls{sv} categories.

Applying ChimeraLM substantially reduced these biases.
The inversion fraction decreased from 83.0\% to 60.7\% on PromethION and from 88.3\% to 46.2\% on MinION, while the relative abundance of deletions and insertions increased toward bulk‑like levels (Fig.~\ref{fig:figure3}f).
By selectively removing artifact‑supported events, ChimeraLM suppresses the dominant inversion inflation and reduces spurious calls across \gls{sv} types, shifting \gls{sv}‑type distributions toward bulk profiles and improving the accuracy and interpretability of single‑cell \gls{sv} analyses~\cite{hardLongreadWholegenomeAnalysis2023a}.
...
\begin{figure}[H]
	...
	\caption{{\bf ChimeraLM attention weights are enriched at chimeric junction regions.}
		...
		During amplification, DNA fragments originating from distant genomic loci can be amplified in either orientation, joining them into a single molecule with discordant orientations, producing inversion-like alignment signatures.
...
As long-context architectures continue to advance~\cite{nguyen2023hyenadna}, extending the model's context window to handle megabase-scale inputs could enable artifact detection in more complex genomic structures.
...
\begin{figure}[!ht]
	...
	\caption{{\bf Composition of artifact-supported \glspl{sv} for PromethION (a) and MinION (b).}
		Donut charts summarize the distribution of \gls{sv} types including \glsfirst{tra}, \glsfirst{del}, \glsfirst{inv}, \glsfirst{dup}, and \glsfirst{ins} among events supported exclusively by chimera artifacts, representing artificial \glspl{sv} preferentially removed by ChimeraLM.}\label{fig:sv_artifact_distribution}
\end{figure}
