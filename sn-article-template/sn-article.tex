...
We therefore compared \gls{sv} type compositions across bulk, unfiltered \gls{wga}, and ChimeraLM-filtered datasets on both nanopore platforms (Fig.~\ref{fig:figure3}f; Extended Data Fig.~\ref{fig:minion_sv_distribution}).
Bulk sequencing showed a balanced mixture of \glspl{del}, \glspl{dup}, \glspl{ins}, \glspl{inv}, and \glspl{tra}.
In contrast, unfiltered \gls{wga} callsets were strongly enriched for \glspl{inv}. ChimeraLM filtering substantially reduced this excess, shifting the \gls{sv}-type profile toward the bulk composition while leaving other \gls{sv} classes comparatively stable.

To determine which \gls{sv} types were preferentially associated with amplification artifacts, we next examined \gls{sv} calls supported exclusively by reads classified as \gls{wga}-induced artifacts (Fig.~\ref{fig:figure3}g; Extended Data Fig.~\ref{fig:minion_sv_artifact_distribution}).
