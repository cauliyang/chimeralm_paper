...
ChimeraLM evaluates reads with chimeric alignments prior to variant calling and classifies each as reflecting a genuine genomic event or an \gls{wga}-induced artifact.
...
These post-filtering rates approach the corresponding bulk baselines, indicating effective removal of \gls{wga}-induced artifacts while preserving authentic genuine signal.
...
We therefore compared \gls{sv} type compositions across bulk, unfiltered \gls{wga}, and ChimeraLM-filtered datasets on both nanopore platforms (Fig.~\ref{fig:figure3}f; Extend Data Fig.~\ref{fig:minion_sv_distribution}).
...
To determine which \gls{sv} types were preferentially associated with amplification artifacts, we next examined \gls{sv} calls supported exclusively by reads classified as \gls{wga}-induced artifacts (Fig.~\ref{fig:figure3}g; Extend Data Fig.~\ref{fig:minion_sv_artifact_distribution}).
...
it reduced chimeric reads by \(\sim\)90\% while retaining 72--92\% of bulk-supported \glspl{sv}, and it increased ratio of supported \gls{sv} calls by 8.5--11.0 fold.
...
CADECT~\cite{agyabeng2025evaluating} achieved only 8.3\% and 4.6\% reduction on PromethION and MinION, respectively, despite being developed for Nanopore data.
CADECT uses sliding-window self-alignment to detect concatemers, reads containing tandem duplications with internal sequence similarity.
This fundamental design limitation explains CADECT's modest performance.
These results demonstrate that heuristics targeting specific artifact subtypes cannot comprehensively address the diversity of amplification-induced chimeras and highlight the need for learning-based models that capture discriminative sequence patterns without relying on predefined structural assumptions.

The efficacy of ChimeraLM highlights the utility of deep learning in quality control tasks where conventional metrics (e.g., mapping quality, read depth) provide limited resolution~\cite{lu2023exploration,kiguchi2021longread,li2024deepchopper, agyabeng2025evaluating}.
