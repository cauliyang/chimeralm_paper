...
\abstract{Single-cell genomic analysis requires \gls{wga} to generate sufficient DNA for sequencing, but this process introduces chimeric artifacts that manifest as false-positive structural variants and compromise downstream analyses.
	...
	ChimeraLM achieved high performance in chimeric detection (F1 score: 0.805, recall: 0.946, precision: 0.700) and dramatically outperformed existing methods, reducing chimeric contamination by $\thicksim 95$\% compared to 0\% for established tools.
...
By removing chimeric sequences upstream of variant calling, ChimeraLM ensures that detected structural variants represent true biological events rather than technical artifacts introduced during amplification (Fig.~\ref{fig:figure1}a).
...
To evaluate ChimeraLM's impact on downstream structural variant analysis, we constructed a gold-standard dataset using bulk sequencing data and assessed \gls{sv} detection performance on both P2 and Mk1c platforms with and without applying ChimeraLM (Fig.~\ref{fig:figure3}a and Fig.~\ref{fig:figure3}b).
This analysis directly measures ChimeraLM's ability to improve the biological relevance of structural variant calls by comparing detected events against high-confidence reference data.
...
ChimeraLM processing dramatically improved this ratio, reducing total SV calls to 305,570 while maintaining 8,067 supported events (2.64\% of total calls).
This represents a 12-fold reduction in the number of structural variants while preserving 91.5\% of true positive events supported by the golden standard, compared with \gls{wga} data.
...
ChimeraLM filtering reduced the total to 38,432 calls while retaining 6,269 supported events, achieving a 16.3\% validation rate and preserving 87.2\% of true positive structural variants compared with \gls{wga} data.
The consistent performance across platforms demonstrates ChimeraLM's robust ability to eliminate false-positive \glspl{sv} while maintaining detection sensitivity for genuine genomic alterations.
...
Unprocessed \gls{wga} data showed dramatically skewed distributions dominated by spurious \gls{inv} events, consistent with literature reports that \gls{inv} are frequently artifactual in amplified samples due to template switching during \gls{wga}~\cite{lu2023exploration, lu2023chimera}.
...
To validate ChimeraLM's classification accuracy, we analyzed the distribution of chimeric alignments per chimeric read, comparing how ChimeraLM classified these known chimeric sequences as biological versus artificial (Extended Data Fig.~\ref{fig:sf2}a and Extended Data Fig.~\ref{fig:sf2}b).
...
Traditional approaches to managing \gls{wga} artifacts have focused on post-hoc filtering of structural variant calls or coverage-based correction methods (cite).
