...
\abstract{Single-cell genomic analysis requires \gls{wga} to generate sufficient DNA for sequencing, but this process introduces chimeric artifacts that manifest as false-positive structural variants and compromise downstream analyses.
	...
	ChimeraLM achieved high performance in chimeric detection (F1 score: \textbf{0.81}) and dramatically outperformed existing methods, reducing chimeric contamination by $\thicksim95$\% compared to 0\% for established tools.
	When applied to \gls{sv} analysis, ChimeraLM improved the validation rate of detected events by \textbf{12-16} fold while preserving $>87$\% of genuine structural variants across multiple Nanopore sequencing platforms.
