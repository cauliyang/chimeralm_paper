...
\title[Article Title]{ChimeraLM: a language model enables accurate structural variant detection in whole-genome amplified long-read sequencing}
...
\abstract{Single-cell genomic analysis relies on \gls{wga} to generate sufficient DNA for sequencing, yet this process introduces chimera artifacts that manifest as false-positive \gls{sv} and compromise downstream analyses.
	Here we present ChimeraLM, an interpretable \gls{glm} that identifies \gls{wga}-induced chimera artifacts directly from sequence information.
	ChimeraLM is trained on matched \gls{wga} and bulk long-read sequencing from the same sample, using bulk support to label chimeric reads as amplification-induced artifacts or genuine genomic events.
	To capture long-range dependencies in variable-length reads, the model integrates Hyena operators with attention pooling.
	Evaluated on matched \gls{wga} and bulk nanopore datasets,
	ChimeraLM eliminated ${\sim}$90\% of chimeric reads from \gls{wga} data, restoring chimeric read proportion to near-bulk levels, whereas existing approaches achieved at most an 8\% reduction.
	Using high-confidence \gls{sv} call sets derived from matched bulk data as a reference, ChimeraLM removed 92-93\% of unsupported \gls{sv} calls from \gls{wga} datasets while retaining 72–92\% of bulk-supported \gls{sv}s.
	ChimeraLM further normalized \gls{sv} type distributions toward bulk profiles by suppressing the characteristic inversion bias observed in unprocessed \gls{wga} data.
	Attention-based interpretation indicates that ChimeraLM concentrates classification evidence at chimeric junctions, demonstrating its ability to learn biologically interpretable features. ChimeraLM provides a general approach for suppressing amplification-induced artifacts, enabling more reliable single-cell \gls{sv} analysis across long-read platforms.}
...
A major source of error in \gls{wga} is amplification-induced chimera formation. During this process, highly processive polymerases such as phi29, which is used in \gls{mda}, can switch templates and join discontinuous genomic loci into a single molecule~\cite{dean2002comprehensive,lu2023chimera,lu2023exploration,lasken2007mechanism,agyabeng2025evaluating}. As a result, \gls{wga}-based sequencing often produces chimeric reads that constitute a substantial fraction of the data~\cite{lu2023chimera}. These artificial sequences frequently create alignment patterns that closely resemble those generated by genuine \gls{sv}s, including \glspl{tra} and \glspl{inv}~\cite{lasken2007mechanism}.
Consequently, \gls{sv} callers that rely on alignment-based signals (e.g., split-read and supplementary alignments) and coverage-derived evidence often misinterpret these amplification artifacts as true rearrangements, inflating false positives and distorting \gls{sv} spectra~\cite{Sedlazeck2018,Smolka2024,chen2023deciphering,heller2019svim,jiang2020longreadbased,gonzalez-pena2021accurate,kosugi2019comprehensive,alkan2011genome, hardLongreadWholegenomeAnalysis2023a}. This problem is particularly consequential for \gls{wga}-based long-read sequencing, which is otherwise well suited for resolving complex \glspl{sv} at sing-cell resolution~\cite{gupta2024Advances,wen2025Singlecell}.

Distinguishing genuine genomic rearrangements from \gls{wga}-induced chimera artifacts remains a major computational challenge.
...
However, these heuristics are often sensitive to platform- and protocol-specific variation.
Moreover, they cannot capture sequence-level patterns or long-range dependencies within reads.
As a result, low-input long-read sequencing remains difficult to deploy in settings where high precision is essential, including somatic mosaicism profiling~\cite{haComprehensiveBenchmarkingGuidelines2023} and validation of CRISPR off-target effects~\cite{hoijerAmplificationfreeLongreadSequencing2020}.

To address this challenge, we present ChimeraLM, an interpretable \gls{glm} for identifying and filtering \gls{wga}-induced artifacts at the single-read level.
Unlike existing approaches that rely on handcrafted rules derived from read alignments or sequence-level~\cite{kiguchi2021longread,lu2023exploration,agyabeng2025evaluating}, ChimeraLM formulates artifact detection as a sequence-modeling task and learns discriminative features directly from raw reads~\cite{li2024deepchopper}.
Building on advances in DNA foundation models~\cite{nguyen2023hyenadna,dalla2025nucleotide,zhou2023dnabert,consens2023transformers}, it captures latent motifs and structural dependencies that generalize across long-read sequencing platforms provided by \gls{ont}.
On \gls{ont} \gls{wga} datasets, ChimeraLM reduces chimeric reads by ${\sim}90\%$ while preserving 72--92\% of bulk-supported \glspl{sv}, improving \gls{sv} validation rates by 8.5- to 11.0-fold and mitigating \gls{wga}-induced biases in \gls{sv} type distributions.
Together, ChimeraLM provides an effective and interpretable filter for \gls{wga} long-read data, enabling robust \gls{sv} discovery in single-cell and low-input genomics.
...
\begin{figure}[p]
	...
	\textbf{(a)}~Single-cell genomic workflow and ChimeraLM integration. Single cells are isolated, followed by DNA extraction and \gls{wga}. During amplification, \gls{wga}-induced chimeric artifacts (red) are generated alongside genuine chimeric reads (green). After base calling and mapping, ChimeraLM classifies reads with chimeric alignments as genuine events or \gls{wga}-induced artifacts, enabling downstream \gls{sv} detection on filtered data.
	\textbf{(b)}~Bulk-supported label generation. Chimeric reads from \gls{wga} data are compared against bulk sequencing from the same cell line. Reads with bulk-supported alignment structures are labeled as genuine events (green); reads with no bulk match are labeled as \gls{wga}-induced artifacts (red).
	\textbf{(c)}~ChimeraLM architecture. Input DNA sequences (batch size $B$, sequence length $L$) are tokenized at single-nucleotide resolution and encoded into hidden states $\mathbf{H} \in \mathbb{R}^{L \times 256}$ through DNA encoder (HyenaDNA~\cite{nguyen2023hyenadna}). Hyena operators capture long-range dependencies. Attention pooling aggregates position-specific features, and \gls{mlp} layers with residual connections process pooled representations for binary classification of genuine events and \gls{wga}-induced artifacts.
...
ChimeraLM operates as a post‑alignment filtering module within the single‑cell long‑read sequencing pipeline (Fig.~\ref{fig:figure1}a). After base calling and alignment to the reference genome, the resulting read set typically contains a mixture of true chimeric reads that arise from authentic genomic rearrangements and artificial chimeras introduced during \gls{wga}.
ChimeraLM evaluates all reads with chimeric alignments before variant calling. For each read, the model determines whether the observed chimera reflects a genuine genomic event or a \gls{wga}‑induced artifact. This binary classification enables selective removal of artificial chimeric reads while preserving true rearrangements, which improves the accuracy and sensitivity of downstream \gls{sv} analyses.

To construct a robust supervised training set, we generated \gls{wga} long-read sequencing data from the human prostate cancer cell line PC3 using the \gls{ont} PromethION platform. For ground-truth calibration, we acquired three matched bulk long-read datasets from unamplified genomic DNA across diverse technologies: \gls{ont} PromethION, \gls{ont} MinION, and \gls{pb} Sequel II.
Leveraging these references, we established a bulk-supported labeling framework by cross-referencing each \gls{wga} chimeric read against the chimeric alignment structures identified in the unamplified bulk data (\hyperref[sec:methods]{Methods}; Fig.~\ref{fig:figure1}b; Extended Data Fig.~\ref{fig:data_construction}a). Under this classification scheme, \gls{wga} reads with alignment architectures corroborated by bulk evidence were labeled as genuine genomic events, while those lacking bulk support were classified as \gls{wga}-induced artifacts. To evaluate the model's ability to generalize across sequencing hardware, we generated an independent \gls{wga} dataset on the MinION platform, which was reserved exclusively for testing (Extended Data Fig.~\ref{fig:data_construction}a).

This labeling procedure classified 12,963,576 chimeric reads from the \gls{wga} PromethION dataset into two groups (genuine events and \gls{wga}-induced artifacts) based on bulk support (Extended Data Fig.~\ref{fig:data_construction}b).
Among these, 12,670,396 reads (97.7\%) showed no matching alignment structures in any bulk dataset and were labeled as \gls{wga}-induced artifacts.
The remaining 293,180 reads (2.3\%) had matching structures in at least one bulk dataset, indicating they represent genuine genomic events rather than amplification artifacts, and were labeled as genuine chimeric reads.
To construct a balanced training dataset, we retained all 293,180 genuine chimeric reads and randomly subsampled an equal number of \gls{wga}-induced artifacts.
We further added 178,748 chimeric reads sampled from the bulk datasets to the genuine-event set, expanding the diversity of bulk-supported chimeric alignment structures used for training.
The final labeled dataset comprised 765,108 reads and was split into training (70\%), validation (20\%), and test (10\%) sets using stratified sampling (Extended Data Fig.~\ref{fig:data_construction}a).

To model these labeled reads, ChimeraLM must process long, variable-length DNA sequences at single-nucleotide resolution (Fig.~\ref{fig:figure1}c).
We therefore built ChimeraLM on HyenaDNA~\cite{nguyen2023hyenadna}, a genomic foundation model pre-trained on diverse DNA sequences.
Each read is tokenized at nucleotide resolution and encoded by Hyena operators~\cite{Poli2023HyenaHT}, which capture long-range sequence context without splitting the input.
The encoder produces a sequence of hidden states across the full read.
To obtain a fixed-length representation for classification, ChimeraLM uses an attention-pooling module that learns position-specific weights and computes a weighted sum over the hidden states (Fig.~\ref{fig:figure1}d).
The pooled representation is then passed through residual \gls{mlp} blocks, and a final softmax outputs the probability that a read reflects a genuine event versus a \gls{wga}-induced artifact.
...
\begin{figure}[!ht]
	...
	\caption{{\bf ChimeraLM accurately identifies and removes \gls{wga}-induced chimeric artifacts.}
		...
		The gray bar indicates the total number of chimeric read on unfiltered \gls{wga} data.
		The blue bar represents the total number of chimeric reads remaining after filtering by each method.
		ChimeraLM achieves approximately 90\% reduction in chimeric reads on both platforms, while 3rd-ChimeraMiner shows no detectable reduction and CADECT shows 8.3\%  and 4.6\% reduction on PromethION and MinION, respectively.
		SACRA failed to complete due to memory exhaustion ($>500$ GB RAM required).
	}\label{fig:figure2}
\end{figure}

\subsection*{ChimeraLM achieves high accuracy and reduces artifacts to near-bulk levels across platforms}

We first benchmarked ChimeraLM using the held-out test split ($10\%$) derived from the bulk-supported labeled dataset (Extended Data Fig.~\ref{fig:data_construction}a).
This test set consists of chimeric reads labeled as genuine events or \gls{wga}-induced artifacts based on whether their chimeric alignment structures are supported by matched bulk sequencing.
ChimeraLM achieved an F1 score of 0.81, with 0.95 recall and 0.70 precision (Fig.~\ref{fig:figure2}a).
The high recall indicates that most \gls{wga}-induced artifacts are correctly identified for removal, which is important for limiting downstream false-positive \gls{sv} calls, while the precision confirms that most reads flagged as artifacts are true amplification-induced chimeras rather than genuine genomic events.

We next examined whether applying ChimeraLM filtering would restore chimeric read rates in full PC3 \gls{wga} datasets toward the levels observed in bulk sequencing across both PromethION and MinION platforms (Fig.~\ref{fig:figure2}b).
Bulk sequencing established low baseline chimeric read rates of 2.3\% (PromethION) and 2.5\% (MinION).
In contrast, \gls{wga} increased the chimeric read fraction to 46.0\% and 23.0\%, respectively.
After ChimeraLM filtering, chimeric content dropped to 4.9\% on PromethION and 1.5\% on MinION, corresponding to 10- to 15-fold reductions, while retaining 15.8 million and 5.6 million reads.
These post-filtering rates approach the corresponding bulk baselines, indicating effective removal of \gls{wga}-induced artifacts while preserving genuine signal.

We compared ChimeraLM against SACRA~\cite{kiguchi2021longread}, 3rd-ChimeraMiner~\cite{lu2023exploration}, and CADECT~\cite{agyabeng2025evaluating}, existing tools for detecting amplification-induced chimeras (Fig.~\ref{fig:figure2}c,d).
ChimeraLM reduced chimeric reads by ${\sim}90$\% on both platforms, outperforming CADECT (8.3\% and 4.6\% reduction on PromethION and MinION, respectively), while 3rd-ChimeraMiner showed no detectable reduction.
SACRA could not be evaluated due to out-of-memory errors even with 500 GB RAM.

The MinION results further provide an independent test of model generalization, as this MinION \gls{wga} dataset was not used during training.
ChimeraLM was trained exclusively on PromethION \gls{wga} data, yet achieved comparable chimeric read reduction on MinION.
This cross-platform generalization indicates that ChimeraLM captures sequence-level features intrinsic to \gls{wga}-induced artifacts rather than platform-specific signatures, supporting its potential applicability to additional long-read and potentially short-read sequencing technologies.
...
\begin{figure}[H]
	...
	\caption{{\bf ChimeraLM improves structural variant detection accuracy.}
		...
		\textbf{f}~\gls{sv} type distributions for PromethION and  MinION across bulk, unfiltered \gls{wga}, and \gls{wga}+ChimeraLM.
		Unfiltered \gls{wga} shows an excess of \glspl{inv}, which is reduced after ChimeraLM filtering.
	}\label{fig:figure3}
\end{figure}

\subsection*{ChimeraLM reduces unsupported structural variant calls}
Accurate \gls{sv} detection from single cells is essential for characterizing genomic diversity and disease mechanisms. However, \gls{wga}‑induced chimeric artifacts can be misinterpreted as genuine \gls{sv}s, potentially leading to incorrect biological inferences. To assess the impact of ChimeraLM on \gls{sv} calling, we compared \gls{sv} callsets generated from unfiltered \gls{wga} reads with those obtained after applying ChimeraLM filtering (\gls{wga} + ChimeraLM). Both callsets were evaluated against two complementary bulk‑derived references: (i) a stringent gold‑standard \gls{sv} set derived from bulk PC3 DNA through cross‑dataset consensus, and (ii) platform‑matched long‑read bulk \gls{sv} callsets used as platform‑specific references.

We first constructed a high-confidence gold-standard \gls{sv} set from bulk PC3 DNA by integrating four independent sequencing datasets: \gls{ont} PromethION, \gls{ont} MinION, and two Illumina short-read \gls{wgs} datasets from the Cancer Cell Line Encyclopedia (CCLE Illumina \gls{wgs}~\cite{barretina2012cancer}) and from a previously published study (PRJNA361315 Illumina \gls{wgs}~\cite{seim2017wholegenome}) (Fig.~\ref{fig:figure3}a; Extended Data Table~\ref{tab:seq_stats}).
\glspl{sv} were called separately within each dataset using multiple \gls{sv} callers.
Events supported by at least two callers within a dataset were retained, and only \glspl{sv} observed in at least two of the four datasets were designated as gold-standard events.

Relative to this stringent gold standard, unfiltered \gls{wga} produced a large number of unsupported \glspl{sv}.
On PromethION, \gls{wga} yielded 3,601,099 \gls{sv} calls, of which only 8,815 (0.24\%) overlapped gold-standard events.
After ChimeraLM filtering, total calls decreased to 305,570 (a 91.5\% reduction) while retaining 8,067 gold-standard events (91.5\% retention), increasing the validation rate to 2.64\% (11-fold) (Fig.~\ref{fig:figure3}b).
On MinION, total calls decreased from 451,390 to 38,432 (a 91.5\% reduction), while gold-standard-supported events decreased from 7,193 to 6,269, corresponding to 87.2\% retention.
The validation rate increased from 1.59\% to 16.3\% (10-fold) (Fig.~\ref{fig:figure3}c).

Because the gold standard is intentionally stringent and may exclude true \glspl{sv} detectable only in long-read data, we next performed platform-matched validation using long-read bulk sequencing from the same platform (Fig.~\ref{fig:figure3}d,e).
...
Together, these results show that ChimeraLM removes the vast majority of unsupported \gls{sv} calls while preserving the majority of bulk-supported variants across platforms.

\subsection*{ChimeraLM mitigates WGA-induced SV type biases}

Amplification artifacts can profoundly distort the apparent spectrum of \gls{sv}s, complicating biological interpretation. To assess these effects, we compared SV‑type compositions across bulk, unfiltered WGA, and ChimeraLM‑filtered datasets generated on both ONT PromethION and MinION platforms. Bulk datasets displayed SV profiles dominated by deletions and insertions, whereas WGA datasets exhibited a striking inflation of inversions, reaching 83.0% on PromethION and 88.3% on MinION. This inversion‑heavy signature mirrors previous observations from single‑cell long‑read sequencing using PacBio and droplet‑based multiple displacement amplification, suggesting that many of these events arise from amplification‑induced chimeras rather than genuine genomic rearrangements.

To directly assess which SV types are preferentially associated with amplification artifacts, we examined SV calls supported exclusively by reads classified as WGA‑derived chimeras. These artifact‑supported events were overwhelmingly enriched for inversions, comprising 88.4% on PromethION and 92.4% on MinION. Smaller fractions of deletions, duplications, and insertions were also present, indicating that WGA artifacts inflate not only inversion calls but generate false positives across multiple SV categories.

Applying ChimeraLM substantially reduced these biases. The inversion fraction decreased from 83.0% to 60.7% on PromethION and from 88.3% to 46.2% on MinION, while the relative abundance of deletions and insertions increased toward bulk‑like levels. By selectively removing artifact‑supported events, ChimeraLM suppresses the dominant inversion inflation and reduces spurious calls across SV types, shifting SV‑type distributions toward bulk profiles and improving the accuracy and interpretability of single‑cell SV analyses.


Amplification artifacts can profoundly distort the apparent spectrum of \gls{sv}s, complicating biological interpretation. To assess these effects, we compared \gls{sv}-type compositions across bulk, unfiltered WGA, and ChimeraLM-filtered datasets generated on both \gls{ont} PromethION and MinION platforms (Fig.~\ref{fig:figure3}f). Bulk datasets exhibited \gls{sv} profiles dominated by deletions and insertions, whereas \gls{wga} datasets showed a striking overrepresentation of inversions (83.0\% on PromethION and 88.3\% on MinION), suggesting widespread amplification-induced artifacts. This inversion bias is consistent with prior observations in single-cell genomes sequenced using PacBio and droplet-based \gls{mda}, where inversions were similarly enriched.

Application of ChimeraLM substantially reduced the inversion fraction, from 83.0\% to 60.7\% on PromethION and from 88.3\% to 46.2\% on MinION. Concurrently, the relative abundance of other \gls{sv} types, particularly deletions and insertions, increased markedly, shifting the overall \gls{sv}-type composition closer to bulk profiles.

To determine which \gls{sv} types were preferentially associated with amplification artifacts, we next examined \gls{sv} calls supported exclusively by reads classified as \gls{wga}-induced artifacts (Extended Data Fig.~\ref{fig:sv_artifact_distribution}).
These artifact-supported events were dominated by \glspl{inv}, accounting for 88.4\% on PromethION and 92.4\% on MinION.
The remaining calls included smaller fractions of \glspl{del} (5.1\% and 3.8\%), \glspl{dup} (3.4\% and 2.4\%), and \glspl{ins} (3.0\% and 1.4\%).
Thus, \gls{wga}-induced artifacts primarily inflate \gls{inv} calls, but also generate false positives across multiple \gls{sv} categories.
By selectively removing artifact-supported events, ChimeraLM restores \gls{sv} type distributions toward bulk profiles, improving the accuracy and interpretability of single-cell \gls{sv} analysis.
...
\gls{wga} enables genomic analysis from single cells and other low-input samples, but it also introduces chimeric artifacts that compromise \gls{sv} detection.
ChimeraLM addresses this challenge by classifying chimeric reads as genuine events or \gls{wga}-induced artifacts directly from read information, and removing artifacts before variant calling, rather than attempting to correct artifact-driven calls post hoc.
Across \gls{ont} platforms, ChimeraLM improved data quality at the read and variant levels:
it reduced chimeric reads by \(\sim\)90\% while retaining 72--92\% of bulk-supported \glspl{sv}, and it increased the ratio of supported \gls{sv} calls by 8.5--11.0-fold.
Notably, performance generalized from PromethION (used for training) to MinION without platform-specific retraining, suggesting that ChimeraLM captures properties shared by \gls{wga}-induced artifacts rather than instrument-specific signatures.

In comparison, existing methods showed limited effectiveness on our \gls{ont} \gls{wga} datasets.
Two tools originally developed and primarily evaluated on PacBio data (SACRA and 3rd-ChimeraMiner)~\cite{kiguchi2021longread,lu2023exploration} either failed to complete under our evaluation setting (SACRA, $>500$ GB RAM) or showed no detectable reduction (3rd-ChimeraMiner), highlighting poor cross-platform generalization.
CADECT~\cite{agyabeng2025evaluating}, which was designed for \gls{ont} data, achieved only 8.3\% and 4.6\% reduction on PromethION and MinION, respectively.
CADECT detects concatemers via sliding-window self-alignment, an effective strategy for repeat-like structures with internal sequence similarity, but it is not designed to capture the broader diversity of \gls{wga}-induced chimeras.
Together, these comparisons suggest that rule-based or subtype-specific heuristics do not comprehensively address amplification-induced chimeras and motivate learning-based models that can extract discriminative sequence signatures without imposing predefined structural assumptions.

ChimeraLM also illustrates the value of deep learning for quality-control problems where conventional alignment- and coverage-derived criteria provide limited resolution.~\cite{lu2023exploration,kiguchi2021longread,li2024deepchopper, agyabeng2025evaluating}.
By learning directly from sequence, ChimeraLM discovers subtle compositional and structural features that separate genuine events from amplification artifacts.
...
These methodological advances have direct implications for single-cell genomics, where high false-positive rates in \gls{wga} data have constrained robust characterization of chromosomal instability, clonal evolution, and \gls{sv} burden~\cite{kosugi2019comprehensive,mahmoud2019structural,hardLongreadWholegenomeAnalysis2023a}.
By improving the signal-to-noise ratio and clarifying \gls{sv}-type spectra that are otherwise distorted by amplification artifacts, ChimeraLM enables more confident identification of genuine \glspl{sv}, supporting studies of cancer evolution, developmental biology, and somatic mosaicism where single-cell resolution is essential~\cite{hoijerAmplificationfreeLongreadSequencing2020, haComprehensiveBenchmarkingGuidelines2023}.

Several limitations warrant consideration.
First, the current model processes reads independently; integrating contextual features such as coverage or phasing information may further enhance accuracy.
Second, regarding computational resources, while \gls{cpu} inference is feasible, \gls{gpu} acceleration is recommended for processing large-scale datasets.
...
While the sequence-based approach suggests broad applicability, systematic validation across amplification chemistries is needed to assess generalization limits and optimize performance for specific protocols.

More broadly, ChimeraLM illustrates the potential of \glspl{glm} for data quality control. As long-context architectures continue to advance~\cite{nguyen2023hyenadna}, extending the model's context window to handle megabase-scale inputs could enable artifact detection in more complex genomic structures.
This framework could extend to other amplification-dependent technologies, such as cell-free DNA analysis, ancient DNA studies, and low-biomass metagenomics.
...
The entire model with ${\sim}4.2$M trainable parameters is trained end-to-end using labeled data.
...
ChimeraLM was compared to existing computational methods for detecting amplification-induced chimeric artifacts: SACRA~\cite{kiguchi2021longread} (GitHub commit~9a2607e),  3rd-ChimeraMiner~\cite{lu2023exploration} (GitHub commit~04b5233), and CADECT v1.2~\cite{agyabeng2025evaluating}.
...
\begin{figure}[!ht]
	...
	\end{center}
	\caption{{\bf Training dataset construction and bulk-supported labeling strategy.}
...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[width=0.9\textwidth]{final_figures/artifacts_suppoorted_sv}
	\end{center}
	\caption{{\bf Composition of artifact-supported \glspl{sv} for PromethION (a) and MinION (b).}
		Donut charts summarize \gls{sv} types among events supported exclusively by chimeric reads, representing artificial \glspl{sv} preferentially removed by ChimeraLM.}\label{fig:sv_artifact_distribution}
\end{figure}
