...
The tokenizer employs character-level encoding where each nucleotide (A, C, G, T, N) is mapped to a unique token ID, with additional special tokens for sequence boundaries and padding.
...
$$
The attention pooling mechanism computes position-wise attention weights through a two-layer neural network where the first linear layer projects from the input hidden dimension (256) to half the classifier hidden dimension (256 dimensions), followed by \gls{gelu} activation and a second linear layer projecting to a single attention score per position.
...
The \gls{mlp} consists of two sequential layers:
First, a linear transformation from input dimension (256) to hidden dimension (512), followed by \gls{gelu} activation and dropout (rate 0.1).
Second, for subsequent layers when input and hidden dimensions match, residual blocks are employed consisting of two linear transformations (512→512) with \gls{gelu} activation and dropout between them, combined with skip connections that add the input to the output.
...
The scheduler monitors validation loss and reduces the learning rate by a factor of 0.1 when no improvement is observed for 10 consecutive epochs, enabling fine-grained optimization as training progresses.
