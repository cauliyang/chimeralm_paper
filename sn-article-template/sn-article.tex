...
\abstract{Single-cell genomic analysis relies on \gls{wga} to generate sufficient DNA for sequencing, yet this process introduces extensive chimeric artifacts that manifest as false-positive \gls{sv} and compromise downstream analyses.
	Here, we developed ChimeraLM, a \gls{glm} that accurately identifies and removes \gls{wga}-induced chimeric artifacts.
	The model architecture integrates Hyena operators with attention mechanisms optimized for variable-length genomic sequences.
	ChimeraLM achieved state-of-the-art performance in chimeric detection (F1 score: \textbf{0.81}) and dramatically outperformed existing approaches, reducing chimeric contamination by $\thicksim95$\% compared to 0\% for established tools.
	When applied to \gls{sv} analysis, ChimeraLM improved the validation rate of detected events by \textbf{12-16} fold while preserving $>87$\% of genuine \gls{sv} across multiple Nanopore sequencing platforms.
	Importantly, ChimeraLM processing normalized \gls{sv} type distributions toward bulk sequencing profiles, eliminating the characteristic false-positive \gls{inv} bias of unprocessed \gls{wga} data.
	This approach directly addresses a fundamental bottleneck in single-cell genomics, enables more reliable genomic analysis of individual cells for applications spanning basic research, clinical diagnostics, and therapeutic development.}
...
Sniffles2~\cite{Sedlazeck2018, Smolka2024}, Manta~\cite{chen2016manta}, DELLY~\cite{rausch2012delly}, DeBreak~\cite{chen2023deciphering}, SVIM~\cite{heller2019svim}, cuteSV~\cite{jiang2020longreadbased}, and SvABA~\cite{wala2018svaba} were used.

\paragraph*{Construction of gold-standard SV dataset}

\subsection*{Figure plotting and visualization}

The figures were created using Python with libraries including Matplotlib~\cite{Hunter2007} and Seaborn~\cite{Waskom2021}.
