...
We therefore compared \gls{sv} type compositions across bulk, unfiltered \gls{wga}, and ChimeraLM-filtered datasets on both \gls{ont} PromethION and MinION datasets. We observed distinct \gls{sv} type distribution profiles between bulk and \gls{WGA} datasets, where indels are abundant in bulk but inversion is the dominat SV types in WGA. The observation of abundatn inversions in our WGA data is consistent with a previous study, which identified inversions and the most abundant SVs from long-read sequencing single cell genomes in CD8+ T cells using PacBio sequencing and droplet-based multiple displacement amplification approach. The abundat of inversion in WGA data suggest it may serve as the major sou


We found INV is the most abundant SV types in both PromethION and MinION WGA datasets, which is consistant with an existing study showing INV is a dominant SV type using pacbio long-read single cell whole genome sequencing data. By applying ChimeraLM filtering, we found that ChimeraLM can significant events to 

(Fig.~\ref{fig:figure3}f).
Bulk sequencing showed a balanced mixture of \glspl{del}, \glspl{dup}, \glspl{ins}, \glspl{inv}, and \glspl{tra}.
In contrast, unfiltered \gls{wga} callsets were strongly enriched for \glspl{inv}.
ChimeraLM filtering reduced this excess, shifting the \gls{sv}-type profile toward the bulk composition while leaving other \gls{sv} classes comparatively stable.
