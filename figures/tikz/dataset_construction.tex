...
\begin{tikzpicture}[node distance=1cm, every node/.style={transform shape}]
	...
	\node[classbox=natred, below=of mdachimeric] (sup0) {
		\begin{tabular}{c}
			\textbf{Support 0}       \\[2pt]
	...
	\node[classbox=natpurple, below = of sup0] (artificial)  {
		\begin{tabular}{c}
	...
	\node[classbox=natblue, below = of sup2] (biological)  {
		\begin{tabular}{c}
	...
	% Add comparison indication with visual grouping
	\node[font=\sffamily\small\bfseries, text=natorange,
		anchor=south] at ($(mdachimeric.north)!0.5!(bulkchimeric2.north)+(0,0.8)$) {
		\begin{tabular}{c}
			Compare MDA reads \\
			with ALL 3 Bulk references
		\end{tabular}
	};
