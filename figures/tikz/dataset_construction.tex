...
\begin{tikzpicture}[node distance=1cm, every node/.style={transform shape}]
	...
	\node[classbox=natred, below= 2cm of mdachimeric] (sup0) {
		\begin{tabular}{c}
			\textbf{Support 0}       \\[2pt]
	...
	% Classification arrows from MDA to support groups - natural curves
	\draw[arrow] (mdachimeric.south) to[out=200,in=110,looseness=0.5] (sup0.north);
	\draw[arrow] (mdachimeric.south) to[out=240,in=110,looseness=0.5] (sup1.north);
	\draw[arrow] (mdachimeric.south) to[out=320,in=110,looseness=0.5] (sup2.north);
	\draw[arrow] (mdachimeric.south) to[out=340,in=110,looseness=0.5] (sup3.north);
	...
	% Bulk data to biological class - smooth curves avoiding overlaps
	% \draw[arrow] (bulkchimeric1.south) to[out=280,in=170,looseness=1.2] (biological.west);
	% \draw[arrow] (bulkchimeric2.south) to[out=270,in=150,looseness=1.0] (biological.north west);
	% \draw[arrow] (bulkchimeric3.south) to[out=260,in=130,looseness=1.2] (biological.north);
